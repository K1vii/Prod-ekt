...
public class WypozyczalniaRowerowApp extends JFrame {
    // --- POLA STATYCZNE (Baza danych w pamięci) ---
    ...
    private void inicjalizujStacje() {
        if (stacjeZainicjalizowane) return;
    ...
    private void setupContentPanel() {
        ...
        // PLiki tekstowe do paneli
        contentPanel.add(simplePanel(wczytajPlik("kontakt.txt", "Błąd odczytu kontaktu.")), "KONTAKT");
        contentPanel.add(simplePanel(wczytajPlik("regulamin.txt", "Błąd odczytu regulaminu.")), "REGULAMIN");

        odswiezMojeWypozyczenia();
    }

    private String wczytajPlik(String nazwaPliku, String blad) {
        StringBuilder tresc = new StringBuilder();
        File plik = new File(nazwaPliku);
        if (plik.exists()) {
            try (BufferedReader br = new BufferedReader(new FileReader(plik))) {
                String linia;
                while ((linia = br.readLine()) != null) tresc.append(linia).append("\n");
            } catch (IOException e) { return blad; }
        } else { return "Brak pliku " + nazwaPliku; }
    ...
    public void odswiezMojeWypozyczenia() {
        ...
        if (czyAdmin) {
            ...
            refreshBtn.addActionListener(e -> { wczytajWypozyczeniaZPliku(); odswiezMojeWypozyczenia(); });
    ...
    public void pokazPanelStacji(String nazwaStacji) {
        ...
        JPanel lista = new JPanel();
        lista.setLayout(new BoxLayout(lista, BoxLayout.Y_AXIS));
        for (Rower r : stacja.getFlota()) lista.add(stworzWizualnyRower(r, stacja));
    ...
    private void runRightSidebarAnimation(boolean open) {
        ...
        rightSidebarTimer = new Timer(10, e -> {
            if (open) {
                ...
                if (rightSidebarWidth >= RIGHT_SIDEBAR_TARGET_WIDTH) { rightSidebarWidth = RIGHT_SIDEBAR_TARGET_WIDTH; rightSidebarTimer.stop(); }
            } else {
                rightSidebarWidth -= 20;
                if (rightSidebarWidth <= 0) { rightSidebarWidth = 0; rightSidebarTimer.stop(); rightSidebar.removeAll(); }
            }
        ...
        sidebarTimer = new Timer(5, e -> {
            if (sidebarWidth != target) {
                ...
                if ((sidebarVisible && sidebarWidth <= target) || (!sidebarVisible && sidebarWidth >= target)) {
                    sidebarWidth = target; sidebarVisible = !sidebarVisible; sidebarTimer.stop();
                }
                sidebar.setPreferredSize(new Dimension(sidebarWidth, getHeight()));
                sidebar.revalidate();
            }
    ...
    private JButton createSidebarButton(String text) {
        ...
        btn.setBackground(new Color(60, 60, 60)); btn.setForeground(Color.WHITE);
        return btn;
    }
    ...
    private JPanel stworzWizualnyRower(Rower r, Wypozyczalnia stacja) {
        ...
        card.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createMatteBorder(0, 0, 1, 0, new Color(220, 220, 220)), BorderFactory.createEmptyBorder(10, 10, 10, 10)));
        ...
        if (r.isDostepny()) {
            rentBtn.setBackground(new Color(39, 174, 96)); rentBtn.setForeground(Color.WHITE);
            rentBtn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        } else {
            rentBtn.setBackground(new Color(149, 165, 166)); rentBtn.setForeground(Color.WHITE);
    ...
    private JPanel simplePanel(String text) {
        ...
        if (!text.trim().startsWith("<html>")) {
            text = "<html><body style='font-family: sans-serif; padding: 10px;'>" + text.replace("\n", "<br>") + "</body></html>";
        }
        area.setText(text); area.setCaretPosition(0);
    ...
    private void zapiszWszystkoDoPliku() {
        try (PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("wypozyczenia.txt", false)))) {
            for (Wypozyczenie w : historiaGlobalna) {
                out.println(w.getKlient().getImie() + ";" + w.getKlient().getNazwisko() + ";" + w.getRower().getModel() + ";" + w.getWypozyczalnia().getNazwa());
            }
    ...
    private void wczytajWypozyczeniaZPliku() {
        ...
        for (Klient k : bazaKlientow.values()) k.getMojeWypozyczenia().clear();
        for (Wypozyczalnia s : stacje.values()) {
            for (Rower r : s.getFlota()) r.setDostepny(true);
        }
                ...
                if (st != null) {
                    for (Rower r : st.getFlota()) {
                        if (r.getModel().equals(d[2])) {
                            ...
                            if (kl == null) { kl = new Klient(d[0], d[1]); bazaKlientow.put(kluczKlienta, kl); }
